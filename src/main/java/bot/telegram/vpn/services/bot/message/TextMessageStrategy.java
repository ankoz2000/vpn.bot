...
import bot.telegram.vpn.services.invest.InvestService;
...
import bot.telegram.vpn.services.vpn.outline.dto.AccessKey;
...
public class TextMessageStrategy implements MessageStrategy<Update, Message> {
    ...
    public SendMessage interactWithReply(UpdateInterface<Update, Message> update) {
        var chatId = update.getMessage().chat().id();
    ...
    private SendMessage getUpro(long chatId) {
        var upro = investService.getUpro();
        return getBaseMessageWithText(chatId, upro);
    }
    ...
    private SendMessage getBaseMessage(Long chatId) {
        return new SendMessage(chatId, "Выбери действие")
                .replyMarkup(new ReplyKeyboardMarkup("Попробовать бесплатно")
                        ...
                        .addRow(new KeyboardButton("UPRO"))
                        .resizeKeyboard(true));
    }
